...
function getWeatherAssets(weatherCondition, isNight) {
    ...
    const assets = {
        ...
        'Partly Cloudy': {
            ...
            icon: isNight ? 'wi-night-partly-cloudy.svg' : 'wi-day-sunny-overcast.svg' 
        }
    };
...
export function createWeatherCard(response) {
    ...
    locationPrecProb.textContent = "Chances of Rain: " + today.values.precipitationProbabilityAvg + "%";
        ...
        for(let i = 0; i < 5; i++) {
            ...
            let temperatureMinValue = response.timelines.daily[i].values.temperatureMin;
                if(units === "fahrenheit") {
                    ...
                    futureDayMinTemp.textContent = Math.round(temperatureMinValue) + (units === "fahrenheit" ? "Fº" : "Cº")
